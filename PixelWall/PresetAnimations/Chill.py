...
class Chill(PresetAnimations.AnimationInstance):
	...
	def Render(self):
		...
		while (len(self.ChillStorage) < Drops):
			self.ChillStorage.append([random.randint(0, self.dFrame.img.size[0]), random.randint(0, self.dFrame.img.size[1]), (random.randint(ColorLower[0], ColorHigher[0]), random.randint(ColorLower[1], ColorHigher[1]), random.randint(ColorLower[2], ColorHigher[2])), random.randint(SpeedMin, SpeedMax), 0])
		col = []
		for i in range(0, len(self.ChillStorage)):
			if self.ChillStorage[i][4] > 255:
				self.ChillStorage[i][4] = 255
			r = int(self.ChillStorage[i][2][0]*(float(COSwave[self.ChillStorage[i][4]])/255))
			g = int(self.ChillStorage[i][2][1]*(float(COSwave[self.ChillStorage[i][4]])/255))
			b = int(self.ChillStorage[i][2][2]*(float(COSwave[self.ChillStorage[i][4]])/255))
			divColor = (r,g,b,255)
