...
class Chill(PresetAnimations.AnimationInstance):
    def Render(self):
        ...
        while (len(ChillStorage) < Drops):
            ChillStorage.append([random.randint(0, self.dFrame.img.size[0]), random.randint(0, self.dFrame.img.size[1]), (random.randint(ColorLower[0], ColorHigher[0]), random.randint(ColorLower[1], ColorHigher[1]), random.randint(ColorLower[2], ColorHigher[2])), random.randint(SpeedMin, SpeedMax), 0])
        col = []
        for i in range(0, len(ChillStorage)):
            ...
            divColor = (r,g,b,255)
            self.dFrame.imgdraw.point([ChillStorage[i][0], ChillStorage[i][1]], divColor)
