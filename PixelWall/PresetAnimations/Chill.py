...
class Chill(PresetAnimations.AnimationInstance):
	...
	def Render(self):
		...
		if ColorLower[0] > ColorHigher[0] or ColorLower[1] > ColorHigher[1] or ColorLower[2] > ColorHigher[2]:
			return
			#raise FAILED TO DETERMINE THE MIN AND MAX COLOR
		...
		while (len(self.ChillStorage) < Drops):
			self.ChillStorage.append([random.randint(0, self.dFrame.img.size[0]-1), random.randint(0, self.dFrame.img.size[1]-1), (random.randint(ColorLower[0], ColorHigher[0]), random.randint(ColorLower[1], ColorHigher[1]), random.randint(ColorLower[2], ColorHigher[2])), random.uniform(SpeedMin, SpeedMax), 0])
		col = []
		for i in range(0, len(self.ChillStorage)):
			...
			divColor = (r,g,b,255)
			self.dFrame.imgdraw.point([self.ChillStorage[i][0], self.ChillStorage[i][1]],fill =  divColor)
