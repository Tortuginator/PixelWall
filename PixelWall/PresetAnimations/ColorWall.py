...
def colorwaves( CRGB* ledarray, numleds, CRGBPalette16& palette):
  sPseudotime = 0;
  sLastMillis = 0;
  sHue16 = 0;
  ...
  ms = datetime.now().microsecond #milis()
...
#Psydo DEFS, since not in FastLED
def scale8(i,scale):
    return int(i*(float(scale)/255))
